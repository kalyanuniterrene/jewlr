                ...
                <div class="tabs-7">
                    <ul class="tabs">
                        ...
                        <li><a href="#tab27">All Sports <span style="display:block;">12</span></a></li><?php */?>
                         <li><a href="#tab32">Match Participant</a></li>
                    </ul>
...
$ch = curl_init("http://cachepricefeeds.williamhill.com/openbet_cdn?action=template&template=getHierarchyByMarketType&classId=1&marketSort=MR&filterBIR=N");
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
curl_setopt($ch, CURLOPT_BINARYTRANSFER, true);
$content = curl_exec($ch);
curl_close($ch);
...
{
	if(substr_count($t['@attributes']['id'],'295')>0)
	{
	...
	//shuffle($names);
	$htmlval ='';
	foreach($names as $makname){
		
	$htmlval .= '<thead><tr>
						<th>'.$makname['@attributes']['name'].'</th>
						<th></th>
						<th></th>
						<th></th>
						<th>Odds</th>
	...
	if($makname['participant']!=null){
		...
		$htmlval .= '<tr>					
					...
					<td>--</td>
					<td>--</td>
					<td>'.$participant['@attributes']['odds'].'</td>
					<td>'.$participant['@attributes']['oddsDecimal'].'</td>
					<td>'.$participant['@attributes']['lastUpdateDate'].'</td>
					<td>'.$participant['@attributes']['lastUpdateTime'].'</td>
					</tr>';
					$a++;
					if($a==5){break;}
		}
	...
	echo $htmlval;
	/*$countmarket[$dss] = count($t->market);
//$htmlcontent[]=array('html'=>getname($t->market),'notype'=>$dss);
echo getname($t->market);*/
	$dss++;
	if($dss==2){
	break;
	}
