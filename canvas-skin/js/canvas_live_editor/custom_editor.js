$(function() {
    ...
    var mirror_image_border_width = jQuery('#width_custom1');

    var photo_wrap_border_width = jQuery('#width_custom2');

    var color_wrap_border_width = jQuery('#width_custom3');



    frame_width = mirror_image_border_width.data('width-border');





    //draggable the canvas

    //=============================



   // jQuery('.image.canvasWrap').draggable({ axis: "x"});





    //draggable room view cancas

    //=============================



    jQuery('.roomCanvasImg').draggable();
    ...
    frame_width = canvasType.data('width-border');
    ...
    jQuery('#collapse3 input[name="selector5"]').on('change', function(argument) {
        ...
        if(selectedValue.photo_retouching == 'minor-image-retouching'){

            jQuery('#major-retouch').slideUp();
            jQuery('#minor-retouch').slideDown();
        }
        else if(selectedValue.photo_retouching == 'major-image-retouching'){ 
            jQuery('#major-retouch').slideDown(); 
             jQuery('#minor-retouch').slideUp();

        }
        else if(selectedValue.photo_retouching == 'no-image-retouching'){  
            jQuery('#major-retouch').slideUp();
            jQuery('#minor-retouch').slideUp();
        }
        ...
        jQuery('#major-retouch textarea').keyup(function(argument) {
            ...
            selectedValue.major_retouching_text = values;
    ...
    invoice_div.find('ul li.subtotal span.price span').text('$' + subTotal);
    ...
    jQuery('.sidebar-content.text button').on('click', function(argument) {
        ...
        if (user_input_text == '') { alert('Oh! It seems you forget to write something!') } else {
    ...
    function Addtext(user_input_text) {
        ...
        selectedValue.canvas_text_options.canvas_text = user_input_text;

        selectedValue.canvas_text_options.fontFamily = 'Arial';

        selectedValue.canvas_text_options.font_color = '#333';

        selectedValue.canvas_text_options.font_size = '30';

        selectedValue.canvas_text_options.is_bold = false;

        selectedValue.canvas_text_options.is_italic = false;

        selectedValue.canvas_text_options.is_underlined = false;
    ...
    jQuery(window).on('load', function(argument) {
       ...
       jQuery('.for_zoom_out').addClass('active'); // activating the canvas zoom 
    ...
    jQuery(document).on('click', '.sidebar-content.size-prize > ul > li', function() {
        ...
        frame_width = jQuery(this).data('width-border');
    ...
    function update_invoice_name(argument) {
        ...
        invoice_div.find('> a').html('(' + argument.canvas_w + 'x' + argument.canvas_h + '” + $' + calculate_and_update_invoice(selectedValue) + ') <i class="fa fa-angle-down" aria-hidden="true"></i> ');

        var rightOne = '<div class="pull-right">' +

            '<a href="#">$ <p>' + argument.canvas_price + '</p></a>' +

            '<a href="#" class="icon"><img src="' + imagePath + 'edit.png" alt="Edit"></a>' +

            '</div>';

        invoice_div.find('ul >li#canvastype').html('<span>'+argument.canvasName+'</span>' + rightOne);

        invoice_div.find('ul li.subtotal span.price span').text('$' + calculate_and_update_invoice(selectedValue));
    ...
    setInterval(function (argument) {
        ...
        set_3d_options();
        if(c_color_new != undefined){
            changeColor2(c_color_new);
        }
    ...
    function setFontsLengths() {
        var length_f = jQuery('.font-list  .tool-select-option.font-option').length;

        jQuery('.tool-category-header.font-category a').eq(0).text( 'All Fonts ( '+length_f+' )');
    }
    ...
    jQuery('.preview-box li').on('click', function (argument) {
    ...
    jQuery(document).on('click', function(e) {
        ...
        if(jQuery('.my-photos-image-box.select').length == 0){
        ...
        if (jQuery('.preview-button').hasClass('active') == true) {
            ...
            if(jQuery('#Wrap-Border ul li.active').length > 0 ){

                if(jQuery('.preview-button .preview-box ul li.active').attr('id') == 'prev_top_left'){
                    top_left_preview();
                }
                //jQuery('.preview-button .preview-box ul li#prev_top_left').addClass('active');
                ...
                alert('You must select an style for the 3D preview');
        ...
        if (!jQuery(e.target).is('.image.canvasWrap , .image.canvasWrap * ')) {
            console.log('aaa');
            jQuery('#contextualEditNav').hide();
            jQuery('#textEditorArea').hide();
            main_Editarea.deactivateAll().renderAll();
          
        }else{
            
           
            if(main_Editarea.getActiveObject().get('type') == 'image'){
                jQuery('#contextualEditNav').show();
            }else{
                jQuery('#textEditorArea').show();
            }
            
            
        }

         if (!jQuery(e.target).is('#textEditorArea, #textEditorArea *, .image.canvasWrap , .image.canvasWrap * ')) {
           // console.log('aaa');
            //jQuery('#contextualEditNav').hide();
          // jQuery('#textEditorArea').hide();
           // main_Editarea.deactivateAll().renderAll();
            ...
            //console.log( main_Editarea.getActiveObject().get('type') );
            //main_Editarea.on('object:selected', onObjectSelected);

            //jQuery('#textEditorArea').show();
        }
        // if (!jQuery(e.target).is('#contextualEditNav , #contextualEditNav *')) {
        //     console.log('cccc');
        // }else{
        //     console.log('dddd');
        // }
    ...
    function swpiaEffect() {
        ...
        // activeO.filters.push(new fabric.Image.filters.Sepia());
    ...
    jQuery('.canvasWrap').append('<div class="loading_img" ><img src="http://beta.canvassignages.com/skin/frontend/rwd/default/images/canvas_live_editor/images/loading.gif" alt=""></div>');
    ...
    function setTheValue(returedValue, canS) {
        ...
        if (returedValue.dataW == 'Custom') {
			...
			var selected_type_s = selectedValue.canvas_type;
			
			if(selected_type_s == "rolled-cannvas"){
				
				tekoW = jQuery('#Rolled-Cannvas .custom-size .select-box select').eq(0).val();

				tekoH = jQuery('#Rolled-Cannvas .custom-size .select-box select').eq(0).siblings().val();
				
				var option_type = 'Rolled Canvas Custom Size';
		
				var url = jQuery('#customsizeDataurl').val();

				//setTheValue(d_datas);
        
				$.ajax({

					url: 'http://beta.canvassignages.com/liveeditor/index/customprice',

					data: { dw: tekoW, dh: tekoH,option_type:option_type },

					type: "POST",

					success: function(response) {
						
						selectedValue.canvas_price = response;
						
						console.log('r: '+selectedValue.canvas_price);
						update_invoice_name(selectedValue);
					}
			...
			if(selected_type_s == "thin_gallery"){
				
				tekoW = jQuery('#Thin-Gallery .custom-size .select-box select').eq(0).val();

				tekoH = jQuery('#Thin-Gallery .custom-size .select-box select').eq(0).siblings().val();

				var option_type = 'Thin Gallery Wrap (0.75) Custom Size';
			...
			if(selected_type_s == "thick_gallery"){
				tekoW = jQuery('#Thick-Gallery .custom-size .select-box select').eq(0).val();

				tekoH = jQuery('#Thick-Gallery .custom-size .select-box select').eq(0).siblings().val();

				var option_type = 'Thick Gallery Wrap (1.5) Custom Size';
    ...
    function setting_default_basedon_canvasType(activeTabContent) {
        ...
        selectedValue.canvas_price = activeTabContent.find('.price-size-table > ul > li.default-selected span.price span').text();
    ...
    $('input[name=selector1]').on('change', function(argument) {
        ...
        if (user_option != custom_option) {
            ...
            selectedValue.canvas_price = ($.trim($('input[name=selector1]:checked').parent().find('span.price span').text()));
    ...
    $('input[name=selector2]').on('change', function(argument) {
        ...
        if (user_option != custom_option) {
            ...
            selectedValue.canvas_price = ($.trim($('input[name=selector2]:checked').parent().find('span.price span').text()));
    ...
    $('input[name=selector3]').on('change', function(argument) {
        ...
        if (user_option != custom_option) {
            ...
            selectedValue.canvas_price = ($.trim($('input[name=selector3]:checked').parent().find('span.price span').text()));
    ...
    // checking if any uploaded img is exists or not
    ...
    setInterval(function() {
        ...
        if (jQuery('.uploaded-image .uploaded-photo-box').length > 0) {

            jQuery('.sidebar-content.image .upload-photo-icon img').hide();

        } else {

            jQuery('.sidebar-content.image .upload-photo-icon img').show();
    ...
    function check_image_exixtance(name) {

        var name_to_match = name;

        jQuery('.uploaded-image .uploaded-photo-box').each(function(argument) {

            if (image_name(jQuery(this).find('img').eq(2).attr('src')) == name_to_match) {
    ...
    jQuery(document).on('click', '.uploaded-photo-box > img:nth-of-type(2)', function() {

        //~ jQuery('.canvasWrap .loading_img').fadeIn();



        dragged_Src = jQuery(this).parent().attr('data-full');
    ...
    jQuery(document).on('click','.remove_image_alt img',function (argument) {
       
        jQuery(this).parent().parent().remove();
        jQuery('.editor-sidebar ul li:nth-of-type(1)').addClass('selected');
    });
    ...
    for (i = 0; i < 99; i++) {
        ...
        jQuery('.randomColors ul').append('<li data-color="' + c_color + '" style="background:' + c_color + '"></li>');
    ...
    jQuery(document).on('click', '.randomColors ul li', function(argument) {
        ...
        selectedValue.canvas_text_options.font_color = jQuery(this).data('color');
    ...
    jQuery(document).on('click', '.for_zoom_in', function(argument) {        
        ...
        jQuery('.image.canvasWrap').css({'transform':'scale('+ scale_ammount +')'});
    ...
    jQuery(document).on('click', '#send2login', function(e) {
		
			var loginEmail = jQuery('#email').val();
			var loginPass = jQuery('#pass').val();
			
			var url = jQuery('#logindataurl').attr('data-url');
           ...
           url: url,
			...
			success: function(response){
					...
					if(response == "login successfully")
					{
						...
						jQuery('.low-res-modal').html(jQuery('.extra_hidden_form').html());
    ...
    //for the browser close time auto save
    ...
    jQuery(document).on('click', '#order_now', function(argument) {
        ...
        var is_custom_edit = jQuery('#is_custom_edit').val();
        
        if(is_custom_edit == 1)
        {
			selectedValue.is_custom = true;
		}
		//~ var custom_edit_text_data = jQuery('#custom_edit_text_data').val();
		//~ if(custom_edit_text_data !="")
		//~ {
			
			//~ selectedValue.canvas_text_options.canvas_text = custom_edit_text_data;

			//~ selectedValue.canvas_text_options.fontFamily = jQuery('.custom_edit_text_data').attr('data-font');

			//~ selectedValue.canvas_text_options.font_color = jQuery('.custom_edit_text_data').attr('data-color');

			//~ selectedValue.canvas_text_options.font_size = jQuery('.custom_edit_text_data').attr('data-size');

			//~ selectedValue.canvas_text_options.is_bold = jQuery('.custom_edit_text_data').attr('data-bold');

			//~ selectedValue.canvas_text_options.is_italic = jQuery('.custom_edit_text_data').attr('data-italic');

			//~ selectedValue.canvas_text_options.is_underlined = jQuery('.custom_edit_text_data').attr('data-underline');
        ...
        var svg = main_Editarea.toDataURLWithMultiplier('svg', 1);

        var svg2 = main_Editarea.toDataURLWithMultiplier('svg', 5);
        ...
        $.ajax({
            ...
            data: { order_img_src: order_img_src, selectedValue: selectedValue },
    ...
    jQuery(document).on('click', '#Wrap-Border li', function(argument) {
        ...
        selectedValue.canvas_style_options = 'wrap border';
    ...
    jQuery(document).on('click', '#Frames li', function(argument) {
        ...
        selectedValue.canvas_style_details = styleName;
        ...
        var frame_price = jQuery(this).attr('data-frame-price');
        ...
        selectedValue.canvas_style_options = 'Frames';
    ...
    function add_style_to_invoice(selectedValue) {       
        ...
        if (selectedValue.canvas_style_price > 0 ) {
        ...
        jQuery('.editor-middle-text ul li#canvasStyle').remove();
    ...
    function add_to_invoice_hardware(selectedValue) {
        ...
        if (selectedValue.hardware_price > 0 ) {
    ...
    function add_to_invoice_filter(selectedValue) {
        ...
        if (selectedValue.filter_price > 0 ) {
    ...
    // invoice click on cross and edit

    //================================



    jQuery(document).on('click', '#canvastype .icon', function(e) {

        e.preventDefault();

        

        setTimeout(function(){ jQuery('.editor-sidebar > ul > li:first-child').addClass('selected'); }, 100);
    ...
    jQuery(document).on('click','#addOnRetouch .icon',function(e) {
		
		e.preventDefault();
		
		jQuery('#collapse3 .radio-group').eq(0).find('input').prop('checked', true);

        jQuery('#collapse3 .radio-group').eq(1).find('input').attr('checked', false);
        
        jQuery('#collapse3 .radio-group').eq(2).find('input').attr('checked', false);
		
		jQuery('#addOnRetouch').remove();
    ...
    jQuery(document).on('click', '#canvasStyle .icon', function(e) {

        e.preventDefault();

        jQuery('#canvasStyle').remove();
        ...
        jQuery('#Frames ul li').each(function(argument) {

            jQuery(this).removeClass('active');

        });
        jQuery('#Wrap-Border ul li').each(function(argument) {
    ...
    jQuery('.design-view').on('click', function(argument) {
        ...
        var c_w = main_Editarea.width;

        var c_h = main_Editarea.height;
        ...
        if (c_w >= c_h) {
            ...
            jQuery('.roomCanvasImg').width(x);

            jQuery('.roomCanvasImg').css({ 'min-width': (x + 20) + 'px' });

            jQuery('.roomCanvasImg').height(200);

        } else if (c_w < c_h) {

            xy = (c_w / c_h) * 200;

            jQuery('.roomCanvasImg').width(xy);

            jQuery('.roomCanvasImg').css({ 'min-width': xy + 'px' });
        ...
        if (selectedValue.canvas_style == 'color_wrap' ) {
            jQuery('.roomCanvasImg').removeClass('no-image');
            jQuery('.roomCanvasImg').css({ 'border-color': selectedValue.canvas_style_details, 'border-image': 'inherit' });

        } else if(selectedValue.canvas_style == 'mirror_image' || selectedValue.canvas_style == 'photo_wrap' ){
            jQuery('.roomCanvasImg').addClass('no-image');

        }
        else {

            jQuery('.roomCanvasImg').removeClass('no-image');
            jQuery('.roomCanvasImg').css({ 'border-image': 'url(' + selectedValue.canvas_style_details + ') 10 round round' });
    ...
    jQuery('.room-view').on('click',function(){

          hide_all_editors(false);
    ...
    hide_all_editors = parameter => parameter == true ? room_view() : design_view();
    ...
    room_view = parameter => {
        ...
        jQuery('#contextualEditNav').fadeOut();
         ...
         jQuery('#textEditorArea').fadeOut();

    }

    design_view = parameter => {



        jQuery('#contextualEditNav').fadeIn();

         jQuery('#textEditorArea').fadeIn();
    ...
    function resizeCanvas(user_custom) {
        ...
        if (input.w > input.h) {
            ...
            $('.canvasWrap').css({'min-width':canvas_final.w+'px'});
    ...
    function make_base() {
        ...
        fabric.Image.fromURL(canvas_background.image, function(imgs) {
            ...
            imgs.scaleToHeight(main_Editarea.getHeight());
    ...
    jQuery('#Rolled-Cannvas .custom-size .select-box select').eq(0).on('change', function(argument) {
        ...
        var dw = jQuery(this).val();
		var dh = jQuery(this).siblings().val();
		var option_type = 'Rolled Canvas Custom Size';
        ...
        $.ajax({
                ...
                data: { dw: dw, dh: dh,option_type:option_type },
    ...
    jQuery('#Rolled-Cannvas .custom-size .select-box select').eq(1).on('change', function(argument) {
        ...
        var dh = jQuery(this).val();
		var dw = jQuery(this).siblings().val();
		var option_type = 'Rolled Canvas Custom Size';
    ...
    function add_image(dragged_Src) {
        ...
        jQuery('.abs_canvas').addClass('resetColor_new');
    ...
    function show_colorPicker(argument) {
        ...
        picker.on("change", function(color) {
            ...
            selectedValue.canvas_style_details = c;
            c_color_new = c;
            changeColor(c);
    ...
    function changeColor(color) {
        ...
        jQuery('.abs_canvas').removeClass('resetColor_new');
    ...
    function resetStyle(styleName) {
        ...
        if (styleName == 'color_wrap') {
            ...
            color_wrap_change();
            jQuery(document).find('.d_sides img').hide();
            jQuery(document).find('.d_sides.front img').show();
            ...
            jQuery(document).find('.d_sides img').show();
    ...
    jQuery('body').append('<canvas id="aa" style="position: absolute;display:none; right:0; z-index: 9999; top: 0;">');
    ...
    function change_mirrors_w() {
        ...
        for (var i = 0; i < allMirrorCanvas.length; i++) {
            ...
            switch (i) {
                ...
                case 0:
                    ...
                    tmcContext.drawImage(img11, frame_width, 0, width11, frame_width, 0, posY, width11, frame_width); // draw the image
                ...
                case 2:
                    ...
                    tmcContext.drawImage(img11, frame_width, height - frame_width, width11, frame_width, 0, posY, width11, frame_width); // draw the image
                ...
                case 3:
                    ...
                    canvas22.height = height - (frame_width * 2);

                    jQuery('#leftCanvas').css({ 'top': frame_width + 'px', 'left': '0px' });

                    //tmcContext.scale(-1, 1); // Set scale to flip the image

                    //posX = frame_width * -1;

                    tmcContext.drawImage(img11, 0, frame_width, frame_width, height, posX, 0, frame_width, height); // draw the image
                ...
                case 1:
                    ...
                    jQuery('#rightCanvas').css({ 'top': frame_width + 'px', 'right': '0px' });

                    //tmcContext.scale(-1, 1); // Set scale to flip the image

                    //posX = frame_width * -1;

                    tmcContext.drawImage(img11, width11 - frame_width, frame_width, frame_width, height, posX, 0, frame_width, height); // draw the image
        ...
        var aaaa = document.getElementById('aa');



        tmcContext = aaaa.getContext("2d"),
        ...
        var www = main_Editarea.width - (frame_width * 2);

        var xxx = main_Editarea.height - (frame_width * 2);

        aaaa.width = www;

        aaaa.height = xxx;

        tmcContext.drawImage(img11, frame_width, frame_width, www, xxx, 0, 0, www, xxx); // draw the image

        var setMain = aaaa.toDataURL('image/png');
    ...
    jQuery('.fontName').on('click', function(argument) {
        ...
        jQuery('.colorPickerArea').addClass('hideFonts');
    ...
    jQuery(document).on('click', '.font-list .tool-select-option.font-option', function(argument) {
       ...
       // setTimeout(function(){ 
       //      main_Editarea.getActiveObject().setFontFamily(jQuery(this).attr('data-font-family-name'));
       ...
       //   }, 1000);       
        ...
        selectedValue.canvas_text_options.fontFamily = jQuery(this).data('font-family-name');
    ...
    jQuery(document).on('click', '.fontSizeArea ul li', function(argument) {
        ...
        selectedValue.canvas_text_options.font_size = jQuery(this).data('font-size');
    ...
    jQuery('.fontSize').on('click', function(argument) {
        ...
        jQuery('.allFontsArea').addClass('hideFonts');
        jQuery('.colorPickerArea').addClass('hideFonts');
    ...
    jQuery('.colorPickerArea .setColorBtn').on('click', function(argument) {
        ...
        selectedValue.canvas_text_options.font_color = jQuery('.colorPickerArea .colorSliders input').val();
    ...
    jQuery('.text-styles div.toolbar-button.bold').on('click', function(argument) {
        ...
        if (jQuery(this).hasClass('selected') === true) {
            ...
            selectedValue.canvas_text_options.is_bold = true;
    ...
    jQuery('.text-styles div.toolbar-button.italic').on('click', function(argument) {
        ...
        if (jQuery(this).hasClass('selected') === true) {
            ...
            selectedValue.canvas_text_options.is_italic = true;
    ...
    jQuery('.text-styles div.toolbar-button.underline').on('click', function(argument) {
        ...
        if (jQuery(this).hasClass('selected') === true) {
            ...
            selectedValue.canvas_text_options.is_underlined = true;
    ...
    jQuery('.editroPlace .editedText').keyup(function(event) {
        ...
        selectedValue.canvas_text_options.canvas_text = jQuery(this).val();
    ...
    jQuery('.order a').on('click', function(argument) {

        //alert(main_Editarea.toDataURLWithMultiplier('png', 3));

        jQuery('body').append('<img class="highRES" src="' + main_Editarea.toDataURLWithMultiplier('png', 5) + '">')
    ...
    jQuery('.sidebar-content.image button.button').on('click',function (argument) {
        jQuery('.fileuploader-items-list li').remove();
         jQuery('.fileuploader-input-caption span').text('Choose files to upload');
         jQuery('#my-photo .button-group button.button.medium.border:first-child').text('Select All');
    });


    jQuery(document).on('mouseup mouseleave mousemove','.color-picker-sv', function (argument) {
       // console.log('ranit');
       jQuery('li.for_styles_tab').addClass('selected');
    })



    jQuery(document).on('click','.editor-middle-text a.dropdown-toggle',function (argument) { 
        
        var ss = jQuery(document).find('#addOnRetouch span').text();
        var sss = ss.replace('-', ' ');
        var ssss = sss.replace('-', ' ');
        jQuery(document).find('#addOnRetouch span').text(ssss);
        jQuery(document).find('#addOnRetouch span').css({'text-transform':'capitalize'});

         var ss2 = jQuery(document).find('#canvasStyle span').text();
        var sss2 = ss2.replace('-', ' ');
        var ssss2 = sss2.replace('-', ' ');
        jQuery(document).find('#canvasStyle span').text(ssss2);
        jQuery(document).find('#canvasStyle span').css({'text-transform':'capitalize'});

    });

    jQuery(document).on('mousedown','.uploaded-photo-box >img',function (argument) {
        jQuery(this).parent().addClass('select').siblings().removeClass('select');
    });


	//alert(selectedValue.canvas_type);
