$(function() {
    ...
    var canvas_background = { image: 'http://beta.canvassignages.com/skin/frontend/revivalpixel/images/canvas_live_editor/images/steel_plate.jpg', text: 'DRAG PHOTO HERE', opacity: 1 };
    ...
    var selectedValue = {
        ...
        canvas_h: undefined,
        is_custom: false,
        fixing_method: { met_name: undefined, met_price: undefined},
    ...
    jQuery(window).on('load', function(argument) {
        ...
        main_Editarea.loadFromJSON((jQuery('#text_data').html()));
        ...
        if(jQuery('.sidebar-content.fixing-method ul li.active').length > 0){
            selectedValue.fixing_method.met_name = jQuery('.sidebar-content.fixing-method ul li.active').data('method-name'); // method name
            selectedValue.fixing_method.met_price = jQuery('.sidebar-content.fixing-method ul li.active').data('method-price'); // method price
            add_to_invoice_fixing_methods(selectedValue);
        }
         if(jQuery('.sidebar-content.steel-plate ul li.active').length > 0){
            var need_to_add = jQuery('.sidebar-content.steel-plate ul li.active img').eq(0).attr('src');
            var json = jQuery(this).attr('data-json');
           if(json == '' || json == undefined || json == 'undefined'){
                 add_image(need_to_add);
             }else{
                 add_from_json(json);
           }
        }
    ...
    function update_invoice_name(argument) {
        ...
        /*invoice_div.find('ul li.subtotal span.price').text('$' + calculate_and_update_invoice(selectedValue));*/
   ...
   // }));
    ...
    jQuery('.canvasWrap .canvas-container')
        .append('<canvas id="topCanvas" class="abs_canvas forWall"></canvas>' +
            '<canvas id="rightCanvas" class="abs_canvas forWall"></canvas>' +
            '<canvas id="bottomCanvas" class="abs_canvas forWall"></canvas>' +
            '<canvas id="leftCanvas" class="abs_canvas forWall"></canvas>'
        );
    ...
    jQuery('.canvasWrap .canvas-container .abs_canvas.forWall').hide();
    ...
    jQuery('.canvasWrap canvas#mainCanvas').droppable({        
         accept: '.sidebar-content.steel-plate img',
    ...
    jQuery(document).on('click', '.saveCanvas', function(e) {
        ...
        savedArea = main_Editarea.toJSON();
        selectedValue.selectedourgallery = jQuery('#selectedourgallery').val();
        savedArea = JSON.stringify(savedArea);
        jQuery('#savedJSONdata').val(savedArea);
    ...
    jQuery(document).on('click', '#fixing_m .icon', function(e) {
        e.preventDefault();
        jQuery('#fixing_m').remove();
        jQuery('.sidebar-content.fixing-method ul li').each(function(argument) {
            jQuery(this).removeClass('active');
        });
        $(".editor-middle-text ul li.subtotal").insertAfter(".editor-middle-text ul li:last-child");
        $(".editor-middle-text ul li.subtotal .price").html('$' + calculate_and_update_invoice(selectedValue));
    ...
    jQuery('.design-view').on('click', function(argument) {
        ...
        if (selectedValue.canvas_style == 'color_wrap') {
            jQuery('.roomCanvasImg').css({ 'border-color': selectedValue.canvas_style_details });
        } else {
    ...
    jQuery(document).on('click','.sidebar-content.fixing-method ul li', function (argument) {
        jQuery(this).addClass('active').siblings().removeClass('active');
        add_method(jQuery(this));
    });

function add_method(argument) {
    var method_name = argument.data('method-name');
    var method_price = argument.data('method-price');
    //alert(method_name);
    selectedValue.fixing_method.met_name = method_name; // method name
    selectedValue.fixing_method.met_price = method_price; // method price

    add_to_invoice_fixing_methods(selectedValue);
}


  function add_to_invoice_fixing_methods(selectedValue) {
        
            
            var rightTwo = '<div class="pull-right">' +
                '<a href="#">$ <p>' + selectedValue.fixing_method.met_price + '</p></a>' +
                '<a href="#" class="icon"><img src="' + imagePath + 'remove.png" alt="Remove"></a>' +
                '</div>';

           // invoice_div.find('ul').append('<li id="fixing_m"><span>' + selectedValue.fixing_method.met_name + '</span>' + rightTwo + '</li>');
            if (jQuery('.editor-middle-text ul li#fixing_m').length > 0) {
                jQuery('.editor-middle-text ul li#fixing_m').html('<span>' + selectedValue.fixing_method.met_name + '</span>' + rightTwo);
            } else {
    ...
    // draggable archive tab
    //==============================

    var accordion = jQuery('#accordion');
    accordion.find('.panel .panel-collapse ul li img').draggable({
        helper: "clone",
        appendTo: 'body',
        zIndex: 9999,
        revert: true,
     ...
     // on-click archive tab
    //==============================

    accordion.find('.panel .panel-collapse ul li img').on('click',function (argument) {
        dragged_Src = jQuery(this).attr('src');
        var json = jQuery(this).parent().parent().attr('data-json');
        console.log(json);       
       if(json == '' || json == undefined || json == 'undefined'){
             add_image(dragged_Src);
         }else{
    ...
    jQuery(document).on('click','.search-result-archive ul li img', function (argument) { 
        jQuery(this).parent().parent().addClass('active').siblings().removeClass('active');
        dragged_Src = jQuery(this).attr('src');
        add_image(dragged_Src);        
    });

    jQuery(document).on('click','.search-result-archive .closeCOmputerModal', function (argument) {             
        jQuery('.search-result-archive').removeClass('show');
    });




    function add_from_json(data) {
        main_Editarea.loadFromJSON(data);
    }
    ...
    function make_base() {
        ...
        fabric.Image.fromURL(canvas_background.image, function(imgs) {
            imgs.width = main_Editarea.width;
            imgs.height = main_Editarea.height;
    ...
    function custom_option_selected(optionType) {
        ...
        if (optionType == 'selector1') {
            custom_size_option.find('li.custom-size select').removeAttr('disabled', true).removeClass('disable');
    ...
    function goForCustomValues(optionType) {
        ...
        if (optionType == 'selector1') {
            ...
            custom_size_option =  jQuery('.sidebar-content.size-prize');

            d_datas.dataW = custom_size_option.find('li.custom-size select').eq(0).val();
            d_datas.dataH = custom_size_option.find('li.custom-size select').eq(1).val();
            setTheValue(d_datas);
    ...
    jQuery('.custom-size .select-box select').eq(0).on('change', function(argument) {
        var d_datas = { dataW: undefined, dataH: undefined };
    ...
    jQuery('.custom-size .select-box select').eq(1).on('change', function(argument) {
        var d_datas = { dataW: undefined, dataH: undefined };
    ...
    jQuery('.order a').on('click', function(argument) {
        ...
        jQuery('body').append('<img src="' + main_Editarea.toDataURLWithMultiplier('png', 5) + '">')
    });
    ...
    jQuery('#search-icon').on('click',function (argument) {

        jQuery('.search-result-archive').addClass('show');
        var url = jQuery(this).data('search-url');
        ...
        $.ajax({
                ...
                {                                    
                    ...
                    jQuery('.search-result-archive').html();
                    jQuery('.search-result-archive').html(response);

                    jQuery('.search-result-archive ul li img').draggable({
                        helper: "clone",
                        appendTo: 'body',
                        zIndex: 9999,
                        revert: true,
